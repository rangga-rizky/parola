	...
	<div>
		...
		<div class="content-header">
			<div class="container-fluid">
				<div class="row mb-2">
					<div class="col-sm-6">
						<h1 class="m-0 text-dark">Daftar Kata Kunci</h1>
					</div><!-- /.col -->
				</div><!-- /.row -->
			</div><!-- /.container-fluid -->
		</div>
					...
					<div class="col-10">					
						<div class="card">
							<div class="card-header">
								 <router-link  :to="{ name: 'terms-create' }"  class="btn btn-success">
										...
										<tr v-for="(term,index) in terms"  v-bind:key="term.id" >
											...
											<td>
												<div class="btn-group" role="group">
  												<router-link  :to="{ name: 'terms-edit', params: {id:term.id}}" type="button" class="btn btn-primary">
    	 ...
    	 data() {
		  ...
		  methods: {
		    fetch() {
		    	...
		    	axios.get('/api/terms?page='+this.paginator.current_page+categoryParam)
			        .then(({data}) => {
			...
			generateTermAssoc() {
				this.pending = true;
		    		axios.post('api/terms/generate-assoc',{
						project_id:1
					})
			        .then(({data}) => {
